import type {
  RegionCooperationTier,
  RegionScalingSnapshot,
} from "./regionScalingTypes";

function getCooperationTier(playerCount: number): RegionCooperationTier {
  if (playerCount <= 1) return "solo";
  if (playerCount === 2) return "pair";
  if (playerCount <= 4) return "group";
  return "crowded";
}

export function buildRegionScalingSnapshot(
  currentMapId: RegionScalingSnapshot["currentMapId"],
