...
function buildWeakPoint(config, index) {
...
export function useMiningMinigame({ config, isOpen, onSuccess, onFailure, }) {
    ...
    const [impactState, setImpactState] = useState("idle");
    const weakPointIndexRef = useRef(0);
    const weakPointTimeoutRef = useRef(null);
    const weakPointRespawnTimeoutRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const tick = (timestamp) => {
            ...
            const nextProgress = clamp(progressRef.current - config.session.idleDecayPerSecond * (deltaMs / 1000), 0, config.session.targetProgress);
    ...
    const handleWeakPointClick = () => {
        ...
        const nextProgress = clamp(progressRef.current + config.session.hitProgress, 0, config.session.targetProgress);
        progressRef.current = nextProgress;
        setProgress(nextProgress);
    ...
    const handleRockMissClick = () => {
        ...
        setCombo(0);
        const nextProgress = clamp(progressRef.current - config.session.missPenalty, 0, config.session.targetProgress);
        progressRef.current = nextProgress;
        setProgress(nextProgress);
    };
    return useMemo(() => ({
