...
function buildHotspot(config, index) {
...
export function useFishingMinigame({ config, isOpen, onCatch, onEscape, }) {
    ...
    const hookRespawnTimeoutRef = useRef(null);
    ...
    useEffect(() => {
        if (!isOpen || phase !== "struggle")
            return;
        const setNextDirectionChange = (elapsedMs) => {
            directionChangeAtRef.current =
                elapsedMs +
                    randomBetween(config.struggle.directionChangeMinMs, config.struggle.directionChangeMaxMs);
        };
        ...
        const tick = (timestamp) => {
            ...
            const fishForce = directionRef.current === "up"
                ? -config.struggle.fishForce
                : config.struggle.fishForce;
            const inputForce = (keyStateRef.current.down ? config.struggle.controlForce : 0) -
                (keyStateRef.current.up ? config.struggle.controlForce : 0);
            const centerForce = (config.struggle.idealZoneCenter - markerPositionRef.current) *
                config.struggle.centerPull;
            markerPositionRef.current = clamp(markerPositionRef.current + (fishForce + inputForce + centerForce) * dt, 0, 100);
            const distanceFromCenter = Math.abs(markerPositionRef.current - config.struggle.idealZoneCenter);
            const idealHalfSize = config.struggle.idealZoneSize / 2;
            if (distanceFromCenter <= idealHalfSize) {
                tensionRef.current = clamp(tensionRef.current + config.struggle.tensionRecoverRate * dt, 0, 100);
            }
            else {
                const overflowFactor = 1 + distanceFromCenter / 30;
                tensionRef.current = clamp(tensionRef.current - config.struggle.tensionDrainRate * overflowFactor * dt, 0, 100);
            }
            const remainingTime = Math.max(0, config.struggle.durationMs - elapsedTimeRef.current);
